...
def _handle_subscription_change(event):
    """サブスクリプション状態変更時の処理"""
    session = event.data.object
...
def _handle_invoice_payment_failed(event, request):
    """支払い失敗時の処理"""
    session = event.data.object
    customer_id = session.customer
