...
### コア
- `users` — ユーザー基本情報（メール、表示名、レベル、XP、ストリーク、提案半径、成人向け施設設定）
...
- `docs/genre-type-design.md` — ジャンル・タイプ設計方針（primaryType→display_type変換フロー・主要定数の役割・ジャンル追加手順）
...
- `プログラミング日誌/_data/Roamble作成日誌.md` — 開発日誌
- `プログラミング日誌/_data/Roamble要件定義書.md` — 要件定義書（Vault内コピー）
