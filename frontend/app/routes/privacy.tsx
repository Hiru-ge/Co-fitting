    ...
    <div className="min-h-screen bg-white dark:bg-bg-dark text-gray-800 dark:text-gray-200">
      ...
      <header className="sticky top-0 z-10 backdrop-blur-md px-4 pt-6 pb-4 border-b border-gray-100 dark:border-white/10 bg-white/80 dark:bg-bg-dark/80">
        <div className="flex items-center justify-between max-w-2xl mx-auto">
          ...
          <h1 className="text-lg font-bold text-center">プライバシーポリシー</h1>
      ...
      <main className="max-w-2xl mx-auto px-5 py-8 space-y-8 text-sm leading-relaxed">
        ...
        <Section number={1} title="取得する情報">
          ...
          <SubSection title="(4) サービス利用に関する情報">
            <ul className="list-disc list-inside space-y-1 ml-2">
              ...
              <li>ゲーミフィケーションデータ（XP、レベル、獲得バッジ、ジャンル習熟度）</li>
              <li>提案半径の設定値</li>
            </ul>
          </SubSection>
          ...
          <SubSection title="(5) アクセス解析情報（導入予定）">
            <p>
              本サービスでは、サービス改善を目的として、Google Analytics（GA4）の導入を予定しています。
        ...
        <Section number={2} title="利用目的">
          ...
          <ul className="list-disc list-inside space-y-1 ml-2 mt-2">
            ...
            <li>ゲーミフィケーション機能の提供（XP付与・レベル管理・バッジ付与）</li>
            <li>サービスの改善・分析</li>
            <li>お問い合わせへの対応</li>
          </ul>
        </Section>
        ...
        <Section number={3} title="Google ユーザーデータの取扱い">
          <p>
            本サービスは、<a
              ...
              rel="noopener noreferrer"
              className="text-primary underline"
            >Google API Services User Data Policy</a>（Limited Use requirements を含む）に準拠しています。
          </p>
          <ul className="list-disc list-inside space-y-1 ml-2 mt-3">
            <li>Google から取得するデータは、メールアドレス・表示名・プロフィール画像URL に限定されます</li>
            <li>これらのデータは、本サービスのアカウント認証・プロフィール表示のためにのみ使用します</li>
            <li>Google ユーザーデータを広告目的で使用することはありません</li>
            <li>Google ユーザーデータを第三者に販売・提供することはありません</li>
            <li>Google ユーザーデータへのアクセスは、サービス提供に必要な最小限の範囲に限定しています</li>
          </ul>
        </Section>

        {/* 4. Cookie・ローカルストレージの利用 */}
        <Section number={4} title="Cookie・ローカルストレージの利用">
          <p>本サービスでは、以下の目的でブラウザのローカルストレージを使用します。</p>
        ...
        <Section number={7} title="安全管理措置">
          <p>運営者は、個人データの漏洩・滅失・毀損を防止するため、以下の措置を講じています。</p>
          <ul className="list-disc list-inside space-y-1 ml-2 mt-2">
            <li>通信の暗号化（HTTPS/TLS）</li>
            <li>認証トークン（JWT）の有効期限管理・リフレッシュトークンによるセッション管理</li>
            <li>パスワードの非保存（Google OAuth認証のため、パスワードはサービス上に存在しません）</li>
            <li>アクセス制御（データベースへのアクセスは認証済みAPIを通じた間接アクセスに限定）</li>
        ...
        <Section number={8} title="データの保存期間">
          ...
          <ul className="list-disc list-inside space-y-1 ml-2 mt-2">
            ...
            <li>ゲーミフィケーションデータ（XP・レベル・バッジ・ジャンル習熟度）</li>
            <li>興味タグの設定</li>
          </ul>
        </Section>
        ...
        <Section number={9} title="ユーザーの権利">
          ...
          <ul className="list-disc list-inside space-y-1 ml-2 mt-2">
            <li><strong>開示請求</strong> — ご自身の個人情報の開示を請求できます</li>
            <li><strong>訂正・追加・削除</strong> — 個人情報の内容が事実でない場合、訂正等を請求できます</li>
