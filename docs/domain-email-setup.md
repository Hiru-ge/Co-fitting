...
## Step 6: Google Search Console でドメイン所有権を確認

GCP OAuth同意画面の本番公開・ブランディング確認に必要。

1. Google Search Console を開く
2. 「プロパティを追加」→ **ドメイン** タイプで `roamble.app` を入力
3. 表示される `google-site-verification=xxxxx` の値をコピー
4. Cloudflare ダッシュボード → `roamble.app` → **DNS** → レコード追加
   - タイプ: `TXT`
   - 名前: `@`
   - 内容: `google-site-verification=xxxxx`（コピーした値）
   - TTL: Auto
5. 数分待ってからSearch Consoleに戻り「確認」をクリック

---

## Step 7: GCP OAuth同意画面のブランディング確認・本番公開

### 7-1. 事前に行うコード修正

**`root.tsx` のベータゲートバイパスに `/privacy` を追加する**

デフォルトでは `/lp` のみバイパス対象のため、`/privacy` に直接アクセスするとベータゲートに弾かれる。
Googleのクローラーも `/privacy` を確認できなくなるため修正が必要。
...
GCP → 「APIとサービス」→「OAuth同意画面」→「ブランディング」

| 項目 | 設定値 | 備考 |
|------|--------|------|
| アプリケーションのホームページ | `https://roamble.app/lp` | rootはSPAのためクローラーが認識しにくい。LPはフッターにプライバシーポリシーリンクあり |
| プライバシーポリシーリンク | `https://roamble.app/privacy` | |
| 承認済みドメイン | `roamble.app` | Search Consoleで所有権確認済みである必要あり |

### 7-3. ブランディング確認の実行

「確認をリクエスト」→「問題は修正した」→「続行」

Googleがサイトをクロールして検証する（数秒〜数分）。

### 7-4. 本番環境に公開

ブランディング確認が通ったら、OAuth同意画面の「対象ユーザー」でステータスを「本番環境に公開」に変更。
これでテストユーザー以外のGoogleアカウントでもログインできるようになる。

---

## 完了後にやること

> チェックリストは `docs/todo.md` に一元管理している。

### GCP設定（`docs/google-oauth-setup.md` も参照）

ドメイン取得後に必要な GCP 側の設定。詳細は `docs/todo.md` の「GCP 設定チェックリスト」を参照。

- OAuth 同意画面 → 承認済みドメインに `roamble.app` を追加
- OAuth 同意画面 → ステータスを「本番環境に公開」に変更（テストユーザー制限の解除）
- OAuthクライアントID → 承認済みJavaScript生成元に `https://roamble.app` を追加
- フロントエンド用APIキー → HTTPリファラー制限に `https://roamble.app/*` を追加
- ドメイン所有権確認: Google Search Console + CloudflareのDNS TXTレコードで対応

### その他

- App Store申請のサポートURL・プライバシーポリシーURLにドメインを使う（Phase 2）
