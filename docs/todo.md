...
**🔴 RED PHASE**
- [ ] `handlers/health_test.go` を作成し、以下のテストを書く
...
**🟢 GREEN PHASE**
- [ ] `database/db.go` に MySQL 接続コード
  - `fmt.Sprintf("...")` で DSN を構築
  - GORM で接続
  - 接続テスト（`Ping()`）
- [ ] `database/migrate.go` にマイグレーション実装
  - `AutoMigrate(&User{}, &Visit{})` で テーブル自動作成
- [ ] `models/user.go` で User 構造体定義
...
- [ ] `models/visit.go` で Visit 構造体定義
- [ ] `main.go` で `db.Init()` → `db.Migrate()` を呼び出し
- [ ] テスト実行 → **通る**
...
**受け入れ基準**
- [ ] `go test ./database -v` で全テスト成功
- [ ] `docker-compose up` で MySQL コンテナが起動
- [ ] `docker-compose logs backend` で接続成功ログが表示
- [ ] MySQL クライアントで `SHOW TABLES;` でテーブルが存在
...
**🟢 GREEN PHASE**
- [ ] `middleware/jwt.go` で認証ミドルウェア実装
  - Authorization ヘッダーから Bearer トークン取得
  - JWT 署名検証
  - トークン期限チェック
  - ユーザー情報を ctx に格納
- [ ] `utils/jwt_helper.go` でトークン生成・検証ユーティリティ
