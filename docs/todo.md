...
**実装内容**
- [ ] `handlers/auth.go`: `SignUp()`, `Login()`, `ChangePassword()` メソッド削除
- [ ] `handlers/auth.go`: `signUpRequest`, `loginRequest`, `changePasswordRequest` 型削除
- [ ] `handlers/user.go`: `UpdateEmail()` メソッド削除
- [ ] `routes/routes.go`: `/signup`, `/login`, `/change-password`, `/users/me/email` ルート削除
- [ ] `handlers/auth_test.go`: SignUp/Login/ChangePassword テスト削除
- [ ] `handlers/user_test.go`: UpdateEmail テスト削除
...
**実装内容**
- [ ] `models/user.go`: `PasswordHash` フィールド削除
- [ ] `database/migrate.go`: `password_hash` カラムドロップのマイグレーション追加
...
**実装内容**
- [ ] `routes/login.tsx`: メール+パスワードフォーム削除、`clientAction()` 削除、Google OAuthボタンのみに
- [ ] `routes/signup.tsx`: 画面削除またはloginへリダイレクト（signup不要のため）
- [ ] `routes/index.tsx`: LP内のsignup/loginリンクを統一
- [ ] `__tests__/routes/login.test.ts`: メール+パスワード関連テスト削除
- [ ] `__tests__/routes/signup.test.ts`: 登録フォーム関連テスト削除
...
**実装内容**
- [ ] `routes/settings.tsx`: メールアドレス変更セクション削除
- [ ] `routes/settings.tsx`: パスワード変更セクション削除
- [ ] `__tests__/routes/settings.test.tsx`: 関連テスト削除
...
**実装内容**
- [ ] `api/users.ts`: `changePassword()`, `updateEmail()` 関数削除
- [ ] `types/auth.ts`: `LoginRequest`, `SignupRequest` 型削除
- [ ] `__tests__/api/users.test.ts`: 関連テスト削除
...
### Issue: 提案エンドポイント気分パラメータ対応
**GitHub Issue**: https://github.com/Hiru-ge/Roamble/issues/129
**優先度**: 🟡 Medium | **工数**: 4h | **担当**: 個人 | **Phase**: Phase 1
**エンドポイント**: `POST /api/suggestions`

**タスク概要**
リクエストボディに `mood` パラメータ（任意）を追加し、気分に応じた施設タイプ優先フィルタリングを実装する。日次キャッシュのキーに気分も含め、気分ごとにキャッシュを独立させる。

**気分と施設タイプマッピング（案）**
- `relax` → カフェ・公園・温泉・書店
- `active` → ジム・スポーツ施設・登山・プール
- `thrill` → バー・ライブハウス・ゲームセンター・脱出ゲーム
- `learn` → 美術館・博物館・図書館・ギャラリー

**TDD実装フロー**
- [x] **RED**: 気分パラメータのテストケース作成
  - 各気分値での施設タイプ絞り込みテスト
  - 気分なし（`mood` 省略）時の従来動作テスト
  - 無効な気分値のバリデーションエラーテスト
  - 気分別キャッシュキー分離のテスト
- [x] **GREEN**: 最小実装
  - リクエストボディに `mood` フィールド追加・バリデーション
  - 気分→施設タイプのマッピングテーブル実装
  - 提案候補フィルタリングに気分マッピングを適用
  - 日次キャッシュキーに `mood` を含める
- [x] **REFACTOR**: リファクタリング
  - マッピングテーブルを設定ファイル化（`moodTypeMapping` として `visitableTypes` と同様の構造で定義）
  - 気分フィルタは結果0件の場合に全施設にフォールバックする設計で優先度調整を実装
...
### Issue: 気分選択UI実装（ホーム画面）
**GitHub Issue**: https://github.com/Hiru-ge/Roamble/issues/130
**優先度**: 🟡 Medium | **工数**: 6h | **担当**: 個人 | **Phase**: Phase 1

**タスク概要**
ホーム画面の提案カード上部に気分プリセット選択UIを追加する。選択した気分は `POST /api/suggestions` のリクエストに `mood` パラメータとして含め、翌日まで保持する（気分を変更したら当日の提案をリフレッシュ）。

**TDD実装フロー**
- [ ] **RED**: 気分選択UIのテストケース作成
  - 4種類のプリセットボタン表示テスト
  - 選択状態のビジュアルフィードバックテスト
  - 気分変更時に提案APIが再リクエストされるテスト
  - 気分未選択時の従来動作テスト
- [ ] **GREEN**: 最小実装
  - ホーム画面に気分プリセット選択コンポーネント追加（リラックス / 体を動かしたい / 刺激がほしい / 学びたい）
  - 選択した気分を `getSuggestions()` の引数として渡す
  - 気分変更時に当日キャッシュを無効化して提案を再取得
  - 選択状態をローカルストレージに保持（ページ更新で消えないよう）
- [ ] **REFACTOR**: リファクタリング
  - 気分アイコン・ラベルの視覚調整
  - アニメーション・UX向上
