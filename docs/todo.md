...
**TDD実装フロー**
- [x] **RED**: メール変更のテストケース作成
  - 認証フローのテスト
  - 重複メールエラーのテスト
- [x] **GREEN**: 最小実装
  - パスワードによる本人確認
  - メールアドレス重複チェック・更新処理
- [x] **REFACTOR**: セキュリティ強化・エラーハンドリング
...
- [ ] **REFACTOR**: リファクタリング
  - スコアリングロジックを独立した関数に分離
  - 興味内/外の混在比率を設定値化（調整容易に）
  - N+1問題対策・興味タグ取得のキャッシュ活用
