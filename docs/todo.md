...
**実装内容**
- [ ] `handlers/suggestion.go`: `mood` フィールドのリクエスト型・バリデーション削除
- [ ] `handlers/suggestion.go`: `moodTypeMapping` マッピングテーブル削除
- [ ] `handlers/suggestion.go`: 提案候補フィルタリングの気分マッピング適用ロジック削除
- [ ] `handlers/suggestion.go`: 日次キャッシュキーから `mood` を除去（気分なし時の従来キーに統一）
- [ ] `handlers/suggestion_test.go`: 気分パラメータ関連テスト削除
