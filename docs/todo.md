...
**🔴 RED PHASE**
- [x] `app/__tests__/utils/constants.test.ts` 作成
  - `import.meta.env.VITE_API_BASE_URL` 設定時の挙動テスト
  - 未設定時のデフォルト値テスト

**🟢 GREEN PHASE**
- [x] `frontend/app/utils/constants.ts` 修正
  ```typescript
  export const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || "http://localhost:8000";
  ```
- [x] `.env.example` に `VITE_API_BASE_URL=http://localhost:8000` 追加

**🔵 REFACTOR PHASE**
- [x] 型安全性の追加（環境変数の型定義）

**受け入れ基準**
- [x] 本番環境でAPIエンドポイントが正しく設定される
- [x] 開発環境では `http://localhost:8000` がデフォルト値として使用される
- [x] ビルドエラーが発生しない
...
**タスク概要**
パスワード長に上限がなく、bcryptの72バイト制限による切り詰めやDoS攻撃のリスクがある。最大72文字制限を追加する。

**🔴 RED PHASE**
- [x] `backend/handlers/auth_test.go` にテスト追加
  - 73文字パスワードでサインアップ失敗テスト
  - パスワード変更時の上限チェックテスト
  - バリデーションエラーメッセージ確認テスト

**🟢 GREEN PHASE**
- [x] `backend/handlers/auth.go` 修正
  ```go
  Password string `json:"password" binding:"required,min=8,max=72"`
  ```
- [x] `changePasswordRequest` の `NewPassword` に同様の制限追加

**🔵 REFACTOR PHASE**
- [x] テスト全体の整合性確認
- [x] bcrypt制限（72バイト）に合わせた実用的な制限値設定

**受け入れ基準**
- [x] 73文字以上のパスワードが拒否される
- [x] 適切なエラーメッセージが返される
- [x] 既存の8文字以上制限は維持される
- [x] bcryptの72バイト制限に対応
...
**🟢 GREEN PHASE**
- [x] `POST /api/auth/logout` リクエスト形式変更
  - リクエストボディに `refresh_token` 追加
- [x] ログアウト処理でリフレッシュトークンもブラックリスト追加
- [x] `RefreshToken` API にブラックリストチェック追加
- [x] フロントエンドの logout 処理更新

**🔵 REFACTOR PHASE**
- [x] トークンブラックリスト管理の共通化（`utils/blacklist.go`）
- [x] TTL管理の最適化（`AddTokenToBlacklist`、`IsTokenBlacklisted`）

**受け入れ基準**
- [x] ログアウト後、リフレッシュトークンが無効化される
- [x] 攻撃者がリフレッシュトークンを保持していても新規アクセストークン生成ができない
- [x] テスト全パス（バックエンド・フロントエンド共）
- [x] 下位互換性を保持（リフレッシュトークンなしでもログアウト可能）

---

### Issue: ListVisits APIの limit パラメータ上限設定 ✅ 完了
**GitHub Issue**: https://github.com/Hiru-ge/Roamble/issues/76
**優先度**: 🟡 Important | **工数**: 0.5h | **担当**: 個人 | **Phase**: Phase 0 Security
**TDD対応**: RED→GREEN→REFACTOR
...
**🔴 RED PHASE**
- [x] `backend/handlers/visit_test.go` にテスト追加
  - `limit=101` で最大100件返却テスト
  - `limit=50` で50件返却テスト
  - 上限超過時の挙動確認テスト

**🟢 GREEN PHASE**
- [x] `backend/handlers/visit.go` の `ListVisits` 修正
  ```go
...
**🔵 REFACTOR PHASE**
- [x] 上限値の定数化（config での管理）

**受け入れ基準**
- [x] limit パラメータが100を超えた場合、100件で制限される
- [x] 通常の取得（100件以下）は正常に動作する
- [x] テスト全パス
...
**🔴 RED PHASE**
- [x] `backend/handlers/suggestion_test.go` にテスト追加
  - `radius=60000` で50000に制限されるテスト
  - `radius=30000` で30000のまま維持されるテスト
  - デフォルト値3000の確認テスト

**🟢 GREEN PHASE**
- [x] `backend/handlers/suggestion.go` 修正
  ```go
  if req.Radius == 0 {
...
**🔵 REFACTOR PHASE**
- [x] 上限値の定数化（`defaultSearchRadius = 3000`, `maxSearchRadius = 50000`）
- [x] コードの可読性向上

**受け入れ基準**
- [x] radius が 50000m を超えた場合、50000m で制限される
- [x] 通常の検索（50000m以下）は正常に動作する
- [x] デフォルト値 3000m は維持される
...
**🔴 RED PHASE**
- [x] `backend/handlers/dev_handler_test.go` にテスト追加
  - JWT認証なしでアクセス拒否テスト
  - 有効なJWTでアクセス成功テスト
  - 開発環境以外での非公開テスト

**🟢 GREEN PHASE**
- [x] `backend/routes/routes.go` 修正
...
**受け入れ基準**
- [x] 未認証でのdev エンドポイントアクセスが拒否される
- [x] 認証済みユーザーのみdev機能を利用できる
- [x] 本番環境では引き続きエンドポイント自体が非公開
...
**A. Critical Priority ✅ 完了**

**🔴 `app/__tests__/routes/history.test.tsx` 作成済み**
- [x] 日付グループ化表示テスト
- [x] カテゴリーフィルター動作テスト
- [x] ページネーション（「もっと見る」）テスト
- [x] 写真URL読み込みテスト
- [x] 空の履歴表示テスト
- [x] エラーハンドリングテスト

**B. Important Priority ✅ 完了**

**🟡 `backend/middleware/cors_test.go` 確認済み（既存）**
- [x] CORS設定の適用確認テスト
- [x] プリフライトリクエスト対応テスト
- [x] 不正オリジンの拒否テスト

**🟡 `backend/middleware/error_handler_test.go` 作成済み**
- [x] 統一エラーレスポンス形式テスト
- [x] HTTP ステータスコード設定テスト
- [x] パニック時の処理テスト

**🟡 `app/__tests__/api/places.test.ts` 作成済み**
- [x] `getPlacePhoto()` 呼び出しテスト
- [x] エラーレスポンス処理テスト
- [x] URLエンコーディングテスト

**🟡 `app/__tests__/api/users.test.ts` 作成済み**
- [x] `updateDisplayName()` API 呼び出しテスト
- [x] `changePassword()` API 呼び出しテスト
- [x] エラーハンドリングテスト

**受け入れ基準 ✅ 全て達成**

- [x] `routes/history.tsx` のテストが19件作成され、主要機能がカバーされる
- [x] middleware の統合テストが追加され、設定ミス検出が可能
- [x] API層のテストが追加され、外部依存の分離が確認される
- [x] 全テストパス: `go test ./...` および `npx vitest run`（**162テスト全成功**）
- [x] テストカバレッジレポートで改善が確認される
