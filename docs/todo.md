...
> Phase 2 の実装は一通り完了。現段階は、自分含むユーザーのFBを元にしたブラッシュアップフェーズ。iOS移行前に対応するのは #321（スキップ機能）・#322（LP英語化のみ）・#344（開拓数露出強化）の3件。#320（効果音）・#345（Receipt of Courage）はiOS版で実装するためPhase 3に移動済み。
