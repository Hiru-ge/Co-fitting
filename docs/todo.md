...
### Issue: Material Symbols アイコンのロード時ちらつき軽減
**GitHub Issue**: https://github.com/Hiru-ge/Roamble/issues/68
**優先度**: 🟡 Medium | **工数**: 1h | **担当**: 個人 | **Phase**: Phase 0

**タスク概要**
material-symbols-outlined を採用したことで、`<span>` タグ内のアイコン画像が読み込まれる前に多少のちらつきが発生している。使用するアイコンを直接ダウンロードしてプロジェクトに配置し、Webフォントではなくローカルアセットとして読み込むことでこのちらつきを軽減する。

**実装内容**
- [ ] 使用中の material-symbols-outlined アイコンをリストアップ
- [ ] 必要なアイコンをSVGまたはフォントファイルとしてダウンロード
- [ ] `frontend/public/` または `frontend/app/assets/` に配置
- [ ] コンポーネント内のアイコン参照をローカルアセットに置き換え
- [ ] Webフォントの外部読み込みを削除（必要に応じて）

**受け入れ基準**
- [ ] ページ初回ロード時のアイコンちらつきが軽減される
- [ ] 全ページでアイコンが正しく表示される
- [ ] 既存のビルド・テストが全パス
