...
**A. バックエンド（Go）**
- [ ] `PATCH /api/users/me` — 表示名更新エンドポイント
- [ ] `POST /api/auth/change-password` — パスワード変更エンドポイント

**B. フロントエンド（React）**
- [ ] `/settings` ルート — ユーザー設定画面
- [ ] 表示名変更フォーム
- [ ] パスワード変更フォーム（現在のパスワード・新しいパスワード・確認入力）
- [ ] マイページの設定ボタンから遷移

**受け入れ基準**
- [ ] 表示名を変更できる
- [ ] パスワードを変更できる（現在のパスワード確認付き）
- [ ] テスト全パス
