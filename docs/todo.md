...
- [ ] `app/utils/devTools.ts` 作成（開発環境のみ）
  ```typescript
...
**受け入れ基準**
- [ ] DELETE `/api/dev/suggestions/cache` が提案キャッシュをリセットする
- [ ] GET `/api/dev/suggestions/stats` が統計情報を返却する
- [ ] 開発環境（`ENVIRONMENT=development`）でのみエンドポイントが有効
- [ ] 本番環境では 404 を返す
- [ ] テスト: `npm test` で handler テスト成功
