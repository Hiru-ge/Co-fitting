...
1. https://render.com にアクセスしてアカウント作成（GitHubログイン）
2. GitHub リポジトリと連携

### 3-2. Web Service 作成

1. 「New」→「Web Service」
2. リポジトリ選択: `Hiru-ge/roamble`（またはfork先）
3. 設定:
   - **Name**: `roamble-backend`
   - **Region**: `Singapore`
   - **Branch**: `main`
...
### 3-3. Dockerfile の本番用ビルド対応

`backend/Dockerfile` は開発用（hot reload）のため、本番用に `backend/Dockerfile.prod` を別途作成済み。

`Dockerfile.prod` の特徴:
- **マルチステージビルド**: `golang:alpine` でビルド → `alpine:latest` に実行バイナリのみコピー（イメージサイズ削減）
- `RUN go build -o roamble main.go` で事前コンパイル済みバイナリを実行（`go run` より高速・安全）
- `ca-certificates` をインストール済み（TiDB / Upstash の TLS 接続に必要）

Render のダッシュボードで以下を設定:
- **Settings → General → Dockerfile Path**: `./Dockerfile.prod`
...
> **JWT_SECRET の生成**: ターミナルで `openssl rand -base64 32` を実行してコピー

### 3-5. TiDB TLS 接続対応（対応済み）

`backend/database/db.go` を修正済み。`MYSQL_TLS` 環境変数の値をそのまま DSN の `tls=` パラメータに渡す実装になっている。

> **注意**: `tls=tidb` は Go の MySQL ドライバーが認識しないカスタム設定名のため使用不可。`tls=true`（システムのルート CA 証明書で検証）を使用すること。`Dockerfile.prod` で `ca-certificates` をインストール済みのため、TiDB Cloud のパブリック証明書を問題なく検証できる。

### 3-6. Redis TLS・パスワード対応（対応済み）

`backend/database/redis.go` を修正済み。

- `REDIS_PASSWORD`: 設定するだけで Upstash 認証が有効になる
- `REDIS_TLS=true`: 設定すると `crypto/tls` を使った TLS 接続が有効になる

> **注意**: Upstash Redis はデフォルトで TLS 接続必須。`REDIS_TLS=true` を設定しないとプレーンTCP接続を試みて `EOF` エラーになる。

### 3-7. UptimeRobot で Render のスリープ防止

Render Free は15分アクセスがないとスリープする。

1. https://uptimerobot.com にアクセスしてアカウント作成(GitHubログイン)
2. 「Add New Monitor」
   - Monitor Type: **HTTP(s)**
   - URL: `https://<render-app-name>.onrender.com/health`
   - Monitoring Interval: **5 minutes**
3. 作成後、Render のデプロイが完了してから有効化
...
| 変数名 | 値 |
|--------|-----|
| `VITE_API_BASE_URL` | `https://<render-app-name>.onrender.com` |
...
> `VITE_API_BASE_URL` の値は Render のデプロイ完了後に確定する（例: `https://roamble-backend.onrender.com`）
